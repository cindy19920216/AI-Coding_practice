...
# --- 1. 디자인 및 스타일 (가로폭 최적화 및 클릭 영역 강제 고정) ---
...
st.markdown("""
    ...
    /* 일상 공유하기 와이드 버튼 (가로폭 100% 및 텍스트 왼쪽 정렬) */
...
def show_sns_page():
    ...
    for i, m in enumerate(family_members):
        with cols[i]:
            if st.button(f"{m['emoji']}\n{m['name']}", key=f"emoji_{m['id']}", use_container_width=True):
...
if st.session_state['user_id'] is None and st.session_state['current_page'] == 'Home':
    ...
    if st.button("❮ 가족 바꾸기 (로그아웃)", key="logout"): 
