...
# Update the end date to None to fetch the most recent data
df = yf.download("SPY", start="2015-01-01", end=None, auto_adjust=True)
df = df[["Close"]].copy()
df.columns = ["Close"]
...
output_dir = os.path.abspath("C:/Users/Check/Desktop/deeplearning_output").replace("\\", os.sep)
try:
    os.makedirs(output_dir, exist_ok=True)
    if not os.path.exists(output_dir):
        raise FileNotFoundError(f"Directory {output_dir} could not be created.")
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "01_ols_vs_deeplearning.png"), dpi=150, bbox_inches="tight")
plt.show()
