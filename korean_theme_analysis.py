...
TICKER_MAP = {
    ...
    '한미반도체':   '042700.KS', 'DB하이텍':         '000990.KS',
    '레인보우로보틱스': '277810.KS', '두산로보틱스':  '454910.KS',
    '한화로보틱스':    '079550.KS', '뉴로메카':        '348370.KQ',
    'HD현대':          '267250.KS',
    'KB금융':       '105560.KS', '신한지주':         '055550.KS',
    '하나금융':     '086790.KS', '우리금융':          '316140.KS',
    'BNK금융':      '138930.KS',
    'HD현대중공업': '329180.KS', '삼성중공업':        '010140.KS',
    '한화오션':     '042660.KS', 'HD현대미포':        '010620.KS',
    'HD한국조선해양': '009540.KS',
    '삼성SDS':      '018260.KS', '포스코DX':          '022100.KS',
    '현대오토에버': '307950.KS',
    'LG에너지솔루션': '373220.KS', '에코프로비엠':    '247540.KS',
    '에코프로':     '086520.KS',   '삼성SDI':          '006400.KS',
    'LG화학':       '051910.KS',
    '한화에어로스페이스': '012450.KS', '현대로템':     '064350.KS',
    'LG생활건강':   '051900.KS', '아모레퍼시픽':      '090430.KS',
    '엔씨소프트':   '036570.KS', '크래프톤':          '259960.KS',
    '넷마블':       '251270.KS',
    '셀트리온':     '068270.KS', '삼성바이오로직스':  '207940.KS',
    'POSCO홀딩스':  '005490.KS', '현대차':            '005380.KS',
    '기아':         '000270.KS', 'NAVER':             '035420.KS',
    '카카오':       '035720.KS',
}
...
def get_naver_theme_map() -> dict:
    print("네이버 테마 + 종목 수집 중...")
    headers, theme_map = {'User-Agent': 'Mozilla/5.0'}, {}
        ...
        try:
            ...
            for link in links:
                ...
                try:
                    ...
                    stocks = [s.text.strip()
                              for s in BeautifulSoup(d.text, 'html.parser').select('td.name a')]
    ...
    print(f"테마 수집 완료: {len(theme_map)}개")
    return theme_map
        ...
        for msg in msgs:
            ...
            for theme, stocks in theme_map.items():
                if theme in STOPWORDS or theme in seen:
                    continue
                if theme in msg:
                    counts[theme] += 1; seen.add(theme); continue
                for stock in stocks:
                    if len(stock) > 1 and stock in msg:
                        counts[theme] += 1; seen.add(theme); break
...
def get_stock_data(monthly_tops: dict, theme_map: dict, ticker_map: dict) -> pd.DataFrame:
    ...
    for theme in all_themes:
        print(f"\n[{theme}] 주가 수집 중...")
        for stock in theme_map.get(theme, []):
            ...
            try:
                ...
                all_data.append(df)
                print(f"  {stock}: {len(df)}일")
                time.sleep(0.3)
            except Exception as e:
                print(f"  {stock} 실패: {e}")

    if not all_data:
        raise ValueError("수집된 주가 데이터가 없습니다.")
    stock_df = pd.concat(all_data, ignore_index=True)
    print(f"\n주가 수집 완료: {stock_df['stock'].nunique()}개 종목")
    return stock_df
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    stock_df = get_stock_data(monthly_tops, theme_map, TICKER_MAP)
    ...
    # STEP 7
    print("\n" + "=" * 60)
    print("STEP 7. 최종 랭킹 + 시각화 (데이터 충분한 테마 위주 필터링)")
    print("=" * 60)
    ...
    # 사용자의 요청에 따라 SI는 포함하고, 은행은 제외
    if 'SI' not in valid_themes: valid_themes.append('SI')
    ...
    merged_df = merged_df[merged_df['theme'].isin(valid_themes)]
